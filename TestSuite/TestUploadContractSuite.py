...
class UploadContract(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.upload = Upload_contract(cls.driver)
    ...
    def upload_pdf(self):
        ...
        self.assertEqual(self.upload.upload_page_toast(),uploaddata[0]['except_result'])
    # 上传doc文件
    def upload_doc(self):
        self.upload.upload_contract(readconfig.url_admin,uploaddata[1]['path'])
        self.assertEqual(self.upload.upload_page_toast(),uploaddata[1]['except_result'])
    # 上传docx文件
    def upload_docx(self):
        self.upload.upload_contract(readconfig.url_admin,uploaddata[2]['path'])
        self.assertEqual(self.upload.upload_page_toast(),uploaddata[2]['except_result'])
    # 上传较大文件
    def upload_75pages(self):
        self.upload.upload_contract(readconfig.url_admin,uploaddata[3]['path'])
        self.assertEqual(self.upload.upload_page_toast(),uploaddata[3]['except_result'])
    #上传空文件
